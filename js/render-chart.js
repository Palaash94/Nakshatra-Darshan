...
// Shared "bling" for every radar chart's data polygon in this app (Life Map, Life Areas, Life
// Balance) - a soft glow behind the filled shape, and its vertex points twinkling gently rather than
// sitting as flat, static dots, so a bright/plain fill colour still reads as "alive" rather than
// bland. seed staggers each point's animation-delay so they glint independently, like real stars,
// instead of all pulsing in unison.
function radarGlowPolygon(pointsStr,fillColor,fillOpacity,strokeColor,strokeWidth,glowColor){
  return`<polygon points="${pointsStr}" fill="${fillColor}" fill-opacity="${fillOpacity}" stroke="${strokeColor}" stroke-width="${strokeWidth}" style="filter:drop-shadow(0 0 6px ${glowColor})"/>`;
}
// A small twinkling circle, deliberately understated - matches the tiny background star dots
// (.chart-star, .const-star-core) used everywhere else in this app rather than standing out as its
// own bold marker. Rendered noticeably smaller than the caller's own r (radarTwinkle, styles.css,
// shrinks it further still via its own animated r), no added glow filter, so it reads as "part of
// the same starfield" rather than a loud UI dot.
function radarGlowPoint(x,y,r,color,seed){
  const delay=((seed*0.37)%2.4).toFixed(2);
  return`<circle class="radar-point" cx="${x}" cy="${y}" r="${Math.max(1.6,r*0.5)}" fill="${color}" style="animation-delay:${delay}s"/>`;
}
...
// Couple-mode only: each partner's Moon Nakshatra constellation sits flanking the "Nakshatra Darshan"
// title itself, at the very top of the page (in normal document flow, so it scrolls away with the
// rest of the hero rather than chasing the user down the page the way the single-person corner badge
// does). No elemental colour tinting here, deliberately - same plain gold/cream palette the rest of
// this app's constellation glyphs already use (.const-line/.const-star-halo/.const-star-core), so the
// two partners are told apart by their actual star patterns, not an added colour. Swapped in whenever
// the Compatibility pill is active (switchActivePerson, app-state.js). Each glyph is a real click
// target - see showConstellationInfo() - opening a short popup on that partner's own birth star,
// reusing the app's existing info-modal (showGenericInfo).
function renderCoupleBackgroundConstellation(){
  if(!chartDataStoreA||!chartDataStoreB)return;
  const a=document.getElementById('hero-const-a'),b=document.getElementById('hero-const-b');
  if(!a||!b)return;
  const nakA=chartDataStoreA.planetData.Moon.nakshatra,nakB=chartDataStoreB.planetData.Moon.nakshatra;
  a.classList.remove('hidden');
  b.classList.remove('hidden');
  a.innerHTML=`<svg viewBox="0 0 200 200" preserveAspectRatio="xMidYMid meet" overflow="visible" onclick="showConstellationInfo('a')" style="cursor:pointer">${constellationInnerSvg(nakA.idx)}</svg>`;
  b.innerHTML=`<svg viewBox="0 0 200 200" preserveAspectRatio="xMidYMid meet" overflow="visible" onclick="showConstellationInfo('b')" style="cursor:pointer">${constellationInnerSvg(nakB.idx)}</svg>`;
  a.title=(chartDataStoreA.name||'Partner A')+"'s birth star";
  b.title=(chartDataStoreB.name||'Partner B')+"'s birth star";
}
// Hides the couple-only hero constellations - called when leaving Compatibility for an individual
// partner's own tab, alongside renderBackgroundConstellation() reclaiming the corner badge.
function hideCoupleBackgroundConstellation(){
  const a=document.getElementById('hero-const-a'),b=document.getElementById('hero-const-b');
  if(a)a.classList.add('hidden');
  if(b)b.classList.add('hidden');
}

// Short, plain-language popup on one partner's own birth star - reuses NAKSHATRA_TRAITS (ephemeris.js)
// and getNakshatraKoota (yogas-doshas-panchang.js, the same Gana/Yoni data Ashtakoot already draws on)
// rather than inventing new content just for this click target.
function showConstellationInfo(person){
  const cd=person==='a'?chartDataStoreA:chartDataStoreB;
  if(!cd)return;
  const nak=cd.planetData.Moon.nakshatra;
  const name=cd.name||(person==='a'?'Partner A':'Partner B');
  const trait=NAKSHATRA_TRAITS[nak.name]||'distinctive';
  const tatva=RASHI_TATVA[cd.planetData.Moon.sign];
  const koota=getNakshatraKoota(nak.idx);
  const text=`<p>${name}'s Moon sits in <strong>${nak.name}</strong> (Pada ${nak.pada}), ruled by ${DASHA_LORD_FULLNAME[nak.lord]}. This birth star reads as ${trait}.</p>
    <p>The Moon itself sits in ${SIGNS[cd.planetData.Moon.sign]}, ${articleFor(tatva)} ${tatva}-element sign.</p>
    <p>Classically this Nakshatra carries a ${koota.ganaInfo.label} temperament (${koota.ganaInfo.desc}) and a ${koota.yoni} instinctual nature.</p>`;
  showGenericInfo(name+"&rsquo;s Birth Star",text);
}
