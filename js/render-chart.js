/* ============================================================
   ...
   segment detail popups, and drishti (aspect) line helpers.
   ============================================================ */

// Generates the SVG markup for drishti (graha aspect) lines connecting each occupied house to every
// house it aspects, drawn at a reduced radius inside the wheel. Benefic-leaning planets (Jupiter, Venus,
// Mercury, Moon) draw in a soft green, others in a soft red - both at low opacity so they read as a subtle
// texture rather than visual clutter. Shared by the D1, D9, and all divisional chart wheels.
function drishtiLinesSvg(houseMap,polarFn,wedgeAnglesFn,radius){
  let out='';
  const seen=new Set();
  for(const planet in ASPECT_OFFSETS){
    ...
    const isBeneficPlanet=['Jupiter','Venus','Mercury','Moon'].includes(planet);
    ASPECT_OFFSETS[planet].forEach(offset=>{
      const toHouse=((fromHouse-1+offset-1)%12)+1;
      if(toHouse===fromHouse)return;
      const key=planet+':'+fromHouse+'-'+toHouse;
      if(seen.has(key))return;
      seen.add(key);
      const fromAng=wedgeAnglesFn(fromHouse).mid,toAng=wedgeAnglesFn(toHouse).mid;
      const p1=polarFn(radius,fromAng),p2=polarFn(radius,toAng);
      const lineColor=isBeneficPlanet?'#4ade8060':'#f8717160';
      out+=`<line x1="${p1.x.toFixed(2)}" y1="${p1.y.toFixed(2)}" x2="${p2.x.toFixed(2)}" y2="${p2.y.toFixed(2)}" stroke="${lineColor}" stroke-width="0.7" pointer-events="none"/>`;
    });
  }
  return out;
}
...
function drawKundli(houseMap,lagnaSign){
  ...
  // Drishti (graha aspect) overlay, drawn at a reduced radius so it reads as texture inside the wheel
  // rather than competing with the spokes, sign icons, and planet positions drawn afterward.
  svgContent+=drishtiLinesSvg(houseMap,polar,wedgeAngles,innerR*0.62);
...
// Draws the North-Indian-style diamond... actually circular wheel shared by
// drawKundli/drawNavamsaChart, with an optional drishti (graha aspect) overlay:
// thin lines from each aspecting planet's house to the house(s) it aspects,
// colored to distinguish benefic-leaning vs malefic-leaning aspects.
// =====================================================================
function drawGenericChartWheel(svgId,houseMap,ascSign,options){
  const opts=options||{};
  const showDrishti=!!opts.showDrishti;
  ...
  // Drishti overlay: thin lines from the center of each occupied house to the center of each house it aspects.
  // Drawn at a reduced radius so lines read clearly inside the wheel without crowding the spokes/labels.
  if(showDrishti){
    svgContent+=drishtiLinesSvg(houseMap,polar,wedgeAngles,innerR*0.62);
  }
