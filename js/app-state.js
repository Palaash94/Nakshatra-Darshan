...
let activePerson='a';
// The last individually-generated chart, kept separate from `chartData` (which gets repointed
// at chartDataStoreA/B while browsing couple mode) so switching reading type and back can
// restore it rather than dropping back to a blank form.
let lastIndividualChart=null;

// Re-selecting a reading type that already has a generated chart restores that chart's result
// view instead of the blank input form, since the underlying data (lastIndividualChart /
// chartDataStoreA+B) was never cleared, only hidden, when the user left via "Change reading type".
function chooseMode(mode){
  currentMode=mode;
  document.getElementById('mode-select').classList.add('hidden');
  // The couple-only hero constellations must never linger into an individual reading - the branches
  // below only reliably hide them when they happen to call renderActiveChart() (which does it as a
  // side effect via renderBackgroundConstellation()), which the FRESH individual form path skips
  // entirely, so this is done unconditionally here instead of relying on that side effect.
  if(mode==='individual')hideCoupleBackgroundConstellation();
  if(mode==='individual'){
    document.getElementById('individual-mode').classList.remove('hidden');
    document.getElementById('couple-mode').classList.add('hidden');
    if(lastIndividualChart){
      chartData=lastIndividualChart;
      ...
      document.getElementById('individual-card').classList.remove('hidden');
      document.getElementById('compat-card').classList.add('hidden');
      showTab('chart');
      renderActiveChart();
      renderYouTab();
    ...
    if(chartDataStoreA&&chartDataStoreB){
      activePerson='a';
      ...
      document.getElementById('switch-btn-compat').classList.remove('active');
      document.getElementById('individual-card').classList.remove('hidden');
      document.getElementById('compat-card').classList.add('hidden');
      showTab('chart');
      renderActiveChart();
      ...
      setTimeout(updateTabScrollButtons,0);
      setTimeout(updateCompatTabScrollButtons,0);
    }else{
  ...
  document.getElementById('switch-btn-compat').classList.toggle('hidden',mode!=='couple');
}

function backToModeSelect(){
  hideCoupleBackgroundConstellation();
  ...
  document.getElementById('name-card').classList.add('hidden');
  document.getElementById('couple-name-card').classList.add('hidden');
  document.getElementById('switch-btn-compat').classList.add('hidden');
  document.getElementById('compat-card').classList.add('hidden');
  document.getElementById('individual-card').classList.remove('hidden');
  currentMode=null;
}

// Gender pill toggle (Couple form only) - drives the real <select id="gender-a/b"> (still what
// calculateCoupleCharts() reads) so the rest of the app doesn't need to know the UI isn't a <select>
// any more. Clicking the already-active pill again clears it back to unspecified, since this field is
// optional and there's no dedicated "clear" control otherwise.
function setGenderPill(person,value){
  const sel=document.getElementById('gender-'+person);
  if(!sel)return;
  const newVal=sel.value===value?'':value;
  sel.value=newVal;
  document.getElementById('gender-btn-'+person+'-female').classList.toggle('active',newVal==='female');
  document.getElementById('gender-btn-'+person+'-male').classList.toggle('active',newVal==='male');
}
...
function computeChartFor(name,dob,tob,lat,lon,tz,place){
  ...
  const cd={name,dob,tob,place:place||'',lat,lon,tz,lagnaSign,ascSid,planetData,houseMap,dashas,panchang,moonNakKoota,ascNakKoota,moonRashiVarna};

  // All 16 divisional (Varga) charts, computed once and shared with both Saptavargaja Bala (below)
  // and the Vargas tab, instead of each independently recomputing/caching its own copy.
  cd.vargaCharts=calcAllDivisionalCharts(planetData,ascSid,lagnaSign,houseMap);

  const weekday=birthDateObj.getDay();
  const hourDecimal=hr+mn/60;
  const sunTimes=calcSunriseSunset(jd,lat,lon,tz);
  const isDayBirth=hourDecimal>=sunTimes.sunriseHr&&hourDecimal<sunTimes.sunsetHr;
  const varshaLordName=varshaLordFromSankranti(jd);
  const navamsaSigns={},retrograde={},declinations={},kendraDists={};
  SHADBALA_PLANETS.forEach(p=>{
    ...
    retrograde[p]=motion.retrograde;kendraDists[p]=motion.kendraDist;
  });
  const birthInfo={hourDecimal,weekday,isDayBirth,navamsaSigns,retrograde,declinations,kendraDists,sunTimes,varshaLordName};
  cd.shadbala=calcShadbala(planetData,houseMap,birthInfo,cd.vargaCharts,ascSid);
  ...
  cd.retrograde=retrograde;
  // Computed eagerly here (renderAvastha, render-tabs.js, also assigns this same field when its own
  // tab is visited, redundant but harmless) rather than left lazy: Life Map/Life Balance and now the
  // couple Life Balance tab need BOTH partners' avasthas even when only one partner's tabs have ever
  // actually been opened, which a purely lazy assignment can't guarantee.
  cd.avasthas=calcAllAvasthas(planetData,planetData.Sun.lon,houseMap);
  return cd;
}
...
function renderActiveChart(){
  ...
  drawKundli(houseMap,lagnaSign);renderPanchang();renderNakshatraKoota();renderPlanetTable(planetData,lagnaSign,ascSid);renderDashas(dashas);renderTrikonaButtons();renderKarakas();renderYogasDoshas();renderShadbala();renderAvastha();drawNavamsaChart();renderNavamsaTable();renderPersonalitySnapshot();renderBigThree();renderChartHeadline();renderChartAtmosphere();renderUpdatesContent();
}
...
function calculateCoupleCharts(){
  ...
  const placeA=document.getElementById('city-a').value.trim(),placeB=document.getElementById('city-b').value.trim();
  chartDataStoreA=computeChartFor(nameA||'Partner A',dobA,tobA,latA,lonA,tzA,placeA);
  chartDataStoreB=computeChartFor(nameB||'Partner B',dobB,tobB,latB,lonB,tzB,placeB);
  // Optional per-partner gender, Couple mode only - blank/unspecified by default, used only to add a
  // gendered framing (husband's Mars/wife's Venus) to the Venus-Mars synastry tab when both partners
  // have opted to set it; every other feature in this app is gender-agnostic and stays that way.
  const genderEl=document.getElementById('gender-a'),genderElB=document.getElementById('gender-b');
  chartDataStoreA.gender=genderEl?genderEl.value:'';
  chartDataStoreB.gender=genderElB?genderElB.value:'';
...
function switchActivePerson(person){
  if(person==='compat'){
    document.getElementById('switch-btn-a').classList.remove('active');
    document.getElementById('switch-btn-b').classList.remove('active');
    document.getElementById('switch-btn-compat').classList.add('active');
    document.getElementById('individual-card').classList.add('hidden');
    document.getElementById('compat-card').classList.remove('hidden');
    setTimeout(updateCompatTabScrollButtons,0);
    renderCoupleBackgroundConstellation();
    return;
  }
  document.getElementById('switch-btn-compat').classList.remove('active');
  document.getElementById('individual-card').classList.remove('hidden');
  document.getElementById('compat-card').classList.add('hidden');
  // Reassigned and the background badge refreshed unconditionally (even when person===activePerson
  // already) so switching Compat -> A -> back always reverts the badge from its couple variant, not
  // just when activePerson genuinely changes.
...
function calculateChart(){
  ...
  errEl.classList.add('hidden');
  chartData=computeChartFor(name,dob,tob,lat,lon,tz,place);
  lastIndividualChart=chartData;
...
function downloadReport(){
  if(!chartData)return;
  const{name,dob,tob,place,lat,lon,tz,lagnaSign,ascSid,planetData,houseMap,dashas,karakas,yogas,doshas,shadbala,bhavaBala,avasthas,navamsa,vargaCharts,panchang,moonNakKoota,ascNakKoota,moonRashiVarna}=chartData;
  const lines=[];
  const now=new Date();
  ...
  lines.push(`- Time of birth: ${tob||'(not provided)'}`);
  lines.push(`- Place of birth: ${place||'(not provided)'}`);
  if(typeof lat==='number'&&!isNaN(lat)&&typeof lon==='number'&&!isNaN(lon)){
    lines.push(`- Coordinates: ${lat.toFixed(4)}° , ${lon.toFixed(4)}°`);
  }
  if(typeof tz==='number'&&!isNaN(tz)){
    lines.push(`- Timezone offset used: UTC${tz>=0?'+':''}${tz}`);
  }
  lines.push('');
  ...
  if(vargaCharts){
    lines.push('## Divisional (Varga) Charts');
    lines.push('(Sign placements across the classical Shodasavarga divisional charts. "Vargottama" marks a planet occupying the same sign in both this varga and the D1 Rashi chart, a significant strength confirmation. D1 and D9/Navamsa are covered in their own sections above.)');
    lines.push('');
    const vargaOrder=['D2','D3','D4','D7','D10','D12','D16','D20','D24','D27','D30','D40','D45','D60'];
    vargaOrder.forEach(key=>{
      const v=vargaCharts[key];
      if(!v)return;
      lines.push(`### ${v.title}`);
      lines.push(v.signifies);
      lines.push(`Ascendant: ${SIGNS[v.ascSign]}`);
      lines.push('');
      lines.push('| Planet | Sign | Vargottama |');
      lines.push('|---|---|---|');
      PLANETS.forEach(p=>{
        lines.push(`| ${p} | ${SIGNS[v.planetSign[p]]} | ${v.vargottama[p]?'Yes':'No'} |`);
      });
      lines.push('');
    });
  }

  lines.push('## Argala & Virodhargala (Jaimini Intervention/Obstruction)');
  lines.push('(Checks whether each house\'s promise is actively supported by planets in its 2nd/4th/11th houses [primary Argala] or 5th house [secondary], and whether that support is obstructed by planets in the mirroring 12th/10th/3rd/9th houses [Virodhargala].)');
  lines.push('');
  const argalaData=calcArgala(lagnaSign,houseMap);
  for(let h=1;h<=12;h++){
    const signIdx=(lagnaSign+h-1)%12;
    lines.push(`- **House ${h} (${SIGNS[signIdx]})**: ${argalaData[h].overall}`);
  }
  lines.push('');
