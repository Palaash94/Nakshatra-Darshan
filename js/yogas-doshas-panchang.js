...
// What each Karaka role "runs through" in a sentence like "With Jupiter as your Atmakaraka,
// ___ runs through wisdom, expansion...".
const KARAKA_DOMAIN_NOUN={
  Atmakaraka:'your soul\'s core lesson this lifetime',
  Amatyakaraka:'the practical vehicle for your life\'s purpose',
  Bhratrukaraka:'the mix of sibling bonds and everyday courage in your chart',
  Matrukaraka:'the mix of your bond with your mother and your sense of home',
  Putrakaraka:'the mix of creativity and connection with children in your chart',
  Gnatikaraka:'the pattern of obstacles and rivalries you keep circling back to',
  Darakaraka:'what you seek, and learn, through marriage'
};

// Closing line per Karaka role, tying the role back to how it tends to show up in daily life.
// Kept separate from the opening/domain sentences so every one of the 63 planet-times-karaka
// combinations still reads as a complete, specific story rather than a swapped-in keyword.
function karakaClosingLine(planet,karaka){
  const closings={
    Atmakaraka:`Whatever ${planet} touches in this chart tends to read less like an ordinary life area and more like the theme your soul keeps circling back to.`,
    Amatyakaraka:`Career and strategic decisions tend to work best when they lean into this rather than fight it.`,
    Bhratrukaraka:`Short efforts, sibling bonds, and everyday bursts of courage all draw on this same well.`,
    Matrukaraka:`What nourishes you, and what you associate with home, traces back to this.`,
    Putrakaraka:`Creativity, children, and whatever past-life merit is ripening now all carry this signature.`,
    Gnatikaraka:`The friction you keep meeting tends to involve this quality, and working with it consciously, rather than against it, is usually the way through.`,
    Darakaraka:`You are drawn to a partner who embodies this, and committed relationships become a mirror for it in yourself.`
  };
  return closings[karaka]||'';
}

// Generates a planet-and-chart-specific narrative for a given Karaka role: what the role means
// through this planet's nature, then grounded in exactly where this planet actually sits (its
// sign, its house, its classical dignity here), not a generic template repeated for every chart.
function karakaPlanetSummary(planet,karaka,chartData){
  const nature=PLANET_NATURE[planet];
  const domain=KARAKA_DOMAIN_NOUN[karaka];
  const d=chartData.planetData[planet];
  const dignity=getDignity(planet,d.sign,d.deg);
  let text=`With ${planet} as your ${karaka}, ${domain} runs through ${nature}.`;
  const seed=Math.round(d.deg*97);
  text+=` It sits in ${SIGNS[d.sign]} here, so that quality shows up as ${signTraitPhrase(d.sign,seed)}, playing out mainly through your ${HOUSE_ORDINAL[d.house]} house: ${HOUSE_THEME_SHORT[d.house]}.`;
  if(dignity)text+=` Classically, ${dignityNoteLine(dignity,seed+11)}, so this thread carries real weight in the chart rather than sitting quietly in the background.`;
  text+=` ${karakaClosingLine(planet,karaka)}`;
  return text;
}
...
// ---------------- VIPARITA RAJA YOGA (Harsha/Sarala/Vimala - dusthana lords combining WITH EACH OTHER) ----------------
// Classical paradox: the 6th/8th/12th (dusthana, "difficulty") house lords are individually unwelcome
// placements, but when two of them conjoin or exchange signs WITH EACH OTHER (rather than touching a
// benefic house), the malefic tendencies are held to cancel out and convert into a gain - success that
// arrives through or after adversity, rather than the plain difficulty either lord would give alone.
function detectViparitaRajaYogas(planetData,lagnaSign){
  const results=[];
  const dusthanaLords=DUSTHANA_HOUSES.map(h=>({house:h,lord:houseLordOf(h,lagnaSign)}));
  const seen=new Set();
  dusthanaLords.forEach(a=>{
    dusthanaLords.forEach(b=>{
      if(a.lord===b.lord||a.house>=b.house)return;
      if(!planetData[a.lord]||!planetData[b.lord])return;
      const key=a.lord+'-'+b.lord;
      if(seen.has(key))return;
      seen.add(key);
      if(planetsConjunct(a.lord,b.lord,planetData)){
        results.push({
          name:`Viparita Raja Yoga (${a.lord}–${b.lord})`,category:'yoga',
          strength:65,strengthLabel:'Moderate',
          formation:`${a.lord} (lord of House ${a.house}) and ${b.lord} (lord of House ${b.house}), both dusthana houses (6th/8th/12th, associated with loss and difficulty), are conjunct in House ${houseOf(a.lord,planetData)}. Classical texts (named Harsha/Sarala/Vimala Yoga for the 6th/8th/12th lord respectively) hold that dusthana lords combining WITH EACH OTHER paradoxically cancels their individual difficulty and produces gain instead.`,
          signifies:'Success that emerges through or after adversity, obstacles, health scares, litigation, or setbacks that ultimately clear the way for unexpected strength, recovery, or victory over rivals and debts, rather than lasting damage.',
          planetsInvolved:[a.lord,b.lord]
        });
      }else{
        const aSign=signOf(a.lord,planetData),bSign=signOf(b.lord,planetData);
        if(SIGN_LORD[aSign]===b.lord&&SIGN_LORD[bSign]===a.lord){
          results.push({
            name:`Viparita Raja Yoga (${a.lord}–${b.lord} exchange)`,category:'yoga',
            strength:70,strengthLabel:'Moderate',
            formation:`${a.lord} and ${b.lord}, lords of dusthana houses ${a.house} and ${b.house}, occupy each other's signs (Parivartana). Two dusthana lords exchanging houses is one of the classical formulas for Viparita Raja Yoga.`,
            signifies:'A pattern of turning difficulty into advantage, reversals of fortune, recovery from loss or illness, or triumph over hidden enemies and obstacles, especially active during these planets\' Dasha periods.',
            planetsInvolved:[a.lord,b.lord]
          });
        }
      }
...
// ---------------- SUNAPHA / ANAPHA / DURUDHARA YOGA (planets flanking the Moon) ----------------
// The positive counterpart to Kemadruma Dosha above - uses the identical "supporting planet" definition
// (excludes Sun, Rahu, Ketu, matching BPHS) so the two features read as consistent opposites of the
// same underlying check rather than two independently-tuned rules.
function detectChandraYogas(planetData){
  ...
  const in2nd=supportingPlanets.filter(p=>houseOf(p,planetData)===secondFromMoon);
  const in12th=supportingPlanets.filter(p=>houseOf(p,planetData)===twelfthFromMoon);
  if(in2nd.length&&in12th.length){
    return{
      name:'Durudhara Yoga',category:'yoga',strength:64,strengthLabel:'Moderate',
      formation:`${in2nd.join(', ')} occupies the 2nd house from the Moon while ${in12th.join(', ')} occupies the 12th, planets flanking the Moon on both sides. This is Durudhara Yoga, the strongest of the three classical Moon-flanking combinations (and the direct opposite condition to Kemadruma Dosha).`,
      signifies:'Material comfort, supportive circumstances, a strong network of resources and allies, and a generally well-provisioned life, the Moon (mind, public standing) is reinforced from both directions rather than isolated.',
      planetsInvolved:[...in2nd,...in12th]
    };
  }
  if(in2nd.length){
    return{
      name:'Sunapha Yoga',category:'yoga',strength:58,strengthLabel:'Mild to moderate',
      formation:`${in2nd.join(', ')} occupies the 2nd house from the Moon (excluding the Sun), the classical formula for Sunapha Yoga.`,
      signifies:'Self-made wealth, intelligence, and reputation built primarily through personal effort rather than inheritance, a capable, resourceful nature.',
      planetsInvolved:in2nd
    };
  }
  if(in12th.length){
    return{
      name:'Anapha Yoga',category:'yoga',strength:58,strengthLabel:'Mild to moderate',
      formation:`${in12th.join(', ')} occupies the 12th house from the Moon (excluding the Sun), the classical formula for Anapha Yoga.`,
      signifies:'Personal grace, good health, and a composed, self-contained temperament, comfort that comes from within rather than external gain-seeking.',
      planetsInvolved:in12th
    };
  }
  return null;
}
...
function detectAdhiYoga(planetData){
  const moonHouse=houseOf('Moon',planetData);
  const benefics=['Mercury','Jupiter','Venus'];
  const targetHouses=[6,7,8].map(off=>((moonHouse-1+off-1)%12)+1);
  const occupied=targetHouses.map(h=>({house:h,planets:benefics.filter(p=>houseOf(p,planetData)===h)})).filter(x=>x.planets.length>0);
  if(!occupied.length)return null;
  const allPlanets=[...new Set(occupied.flatMap(o=>o.planets))];
  const houseCount=occupied.length;
  return{
    name:'Adhi Yoga',category:'yoga',
    strength:50+houseCount*10,strengthLabel:houseCount>=3?'Strong':houseCount===2?'Moderate':'Mild',
    formation:`${occupied.map(o=>`${o.planets.join(', ')} in House ${o.house}`).join('; ')}, benefic planet(s) occupying the 6th, 7th, and/or 8th house counted from the Moon. This is Adhi Yoga, present here across ${houseCount} of the 3 classically checked positions.`,
    signifies:'Leadership, authority, and a life marked by respect and rising status, classically said to make one comparable to a minister or commander, with the strength of the yoga scaling with how many of the three positions are occupied.',
    planetsInvolved:allPlanets
  };
}

// ---------------- AMALA YOGA (unafflicted benefic alone in the 10th from Lagna or Moon) ----------------
function detectAmalaYoga(planetData,lagnaSign){
  const benefics=['Jupiter','Venus','Mercury'],malefics=['Sun','Mars','Saturn','Rahu','Ketu'];
  const moonHouse=houseOf('Moon',planetData);
  const tenthFromMoon=((moonHouse-1+9)%12)+1;
  function checkTenth(fromLabel,tenthHouse){
    const occupants=PLANETS.filter(p=>houseOf(p,planetData)===tenthHouse);
    const beneficsHere=occupants.filter(p=>benefics.includes(p));
    const hasMalefic=occupants.some(p=>malefics.includes(p));
    if(beneficsHere.length&&!hasMalefic)return{fromLabel,house:tenthHouse,planets:beneficsHere};
    return null;
  }
  const hit=checkTenth('Ascendant',10)||checkTenth('Moon',tenthFromMoon);
  if(!hit)return null;
  return{
    name:'Amala Yoga',category:'yoga',strength:56,strengthLabel:'Mild to moderate',
    formation:`${hit.planets.join(', ')} occupies House ${hit.house}, the 10th from the ${hit.fromLabel}, with no malefic planet present there. An unafflicted benefic alone in the 10th (career/public reputation) house is Amala Yoga.`,
    signifies:'A clean, lasting reputation, actions and achievements that hold up to scrutiny over time, and a career or public standing built on genuine merit rather than controversy.',
    planetsInvolved:hit.planets
  };
}
...
function detectAllYogas(planetData,lagnaSign,houseMap){
  ...
  all=all.concat(detectViparitaRajaYogas(planetData,lagnaSign));
  const cy=detectChandraYogas(planetData);if(cy)all.push(cy);
  const ad=detectAdhiYoga(planetData);if(ad)all.push(ad);
  const am=detectAmalaYoga(planetData,lagnaSign);if(am)all.push(am);
...
// ---------------- MANGAL (KUJA) DOSHA ----------------
// Mars in houses 1,2,4,7,8,12 from the Lagna. The two reference points that actually determine
// whether the dosha is considered PRESENT, matching how mainstream calculators (including
// AstroSage, this app's own stated reference point) treat it, are the Ascendant and the Moon.
// Venus is a genuine, if far less commonly applied, classical third check (Shukra Kuja Dosha) -
// it used to independently trigger the flag here too, which meant charts with Mars nowhere near
// a dosha house from Lagna or Moon (e.g. Mars simply sitting in the 3rd from the Ascendant) could
// still come back "Mangal Dosha present" purely on the Venus check, a result that reads as wrong
// against every mainstream reference. Venus is now folded in only as supporting context once the
// dosha is already triggered from Lagna or Moon, never as a trigger on its own.
...
function detectMangalDosha(planetData,lagnaSign){
  ...
  // Each reference point (Ascendant/Moon/Venus) counts Mars's house differently, so each needs its own
  // house number here - a previous version always showed marsHouseFromLagna next to whichever reference
  // point(s) triggered, which produced a mismatched/wrong house number whenever Moon or Venus triggered
  // the dosha without the Ascendant also doing so.
  const refDescs=[];
  if(fromLagna)refDescs.push(`House ${marsHouseFromLagna} from the Ascendant`);
  if(fromMoon)refDescs.push(`House ${marsHouseFromMoon} from the Moon`);
  if(fromVenus)refDescs.push(`House ${marsHouseFromVenus} from Venus`);
  const refText=refDescs.length===1?refDescs[0]:refDescs.slice(0,-1).join(', ')+', and '+refDescs[refDescs.length-1];
  return{
    ...
    formation:`Mars falls in ${refText}, one of the six houses (1, 2, 4, 7, 8, 12) classically used to assess Mangal Dosha, primarily examined for marital compatibility and timing.${cancellations.length?' However: '+cancellations.join('; ')+'.':' No major classical cancellation factors are present in this chart.'}`,
...
// ---------------- DARIDRA YOGA (11th lord in a dusthana) ----------------
function detectDaridraYoga(planetData,lagnaSign){
  const l11=houseLordOf(11,lagnaSign);
  if(!planetData[l11])return null;
  const l11House=houseOf(l11,planetData);
  if(![6,8,12].includes(l11House))return null;
  return{
    name:'Daridra Yoga',category:'dosha',strength:48,strengthLabel:'Mild',
    formation:`The 11th house lord (${l11}, gains and income) is placed in House ${l11House}, one of the three dusthana (loss/difficulty) houses, a classical marker of financial strain or income that doesn't accumulate as expected.`,
    signifies:'A tendency for income to leak away as fast as it arrives, unexpected expenses, or effort that doesn\'t convert to lasting gain as easily as the chart might otherwise suggest, usually most active during this planet\'s Dasha periods, and very commonly offset by any Dhana or Raja Yoga elsewhere in the same chart.',
    planetsInvolved:[l11]
  };
}

// ---------------- GRAHAN DOSHA (eclipse - Sun or Moon conjunct a lunar node) ----------------
function detectGrahanDosha(planetData){
  const pairs=[['Sun','Rahu'],['Sun','Ketu'],['Moon','Rahu'],['Moon','Ketu']];
  const hit=pairs.find(([a,b])=>planetsConjunct(a,b,planetData));
  if(!hit)return null;
  const[luminary,node]=hit;
  return{
    name:'Grahan Dosha',category:'dosha',strength:56,strengthLabel:'Mild to moderate',
    formation:`${luminary} is conjunct ${node} in House ${houseOf(luminary,planetData)} (${SIGNS[planetData[luminary].sign]}), the shadow planet ${node} eclipsing ${luminary==='Sun'?'the Sun (self, vitality, father)':'the Moon (mind, mother, emotional security)'}, the classical eclipse configuration.`,
    signifies:`A tendency toward self-doubt, an obscured sense of ${luminary==='Sun'?'identity or purpose':'emotional clarity'}, or health/confidence themes connected to ${luminary}, that often clear noticeably once the person moves past ${node}'s associated Dasha periods.`,
    planetsInvolved:[luminary,node]
  };
}

// ---------------- VISH YOGA (Saturn-Moon conjunction) ----------------
function detectVishYoga(planetData){
  if(!planetsConjunct('Saturn','Moon',planetData))return null;
  return{
    name:'Vish Yoga',category:'dosha',strength:50,strengthLabel:'Mild',
    formation:`Saturn and the Moon are conjunct in House ${houseOf('Moon',planetData)} (${SIGNS[planetData.Moon.sign]}), literally "poison yoga," Saturn's restrictive influence sitting directly on the mind-signifying Moon.`,
    signifies:'A tendency toward anxiety, emotional heaviness, or a pessimistic inner narrative that doesn\'t always match outer circumstances, often eases substantially with age as Saturn\'s disciplining influence matures into resilience rather than restriction.',
    planetsInvolved:['Saturn','Moon']
  };
}

// ---------------- SHAKATA YOGA (Jupiter 6th or 8th from Moon) ----------------
function detectShakataYoga(planetData){
  const moonHouse=houseOf('Moon',planetData);
  const jupHouse=houseOf('Jupiter',planetData);
  const dist=houseDistance(moonHouse,jupHouse);
  if(dist!==6&&dist!==8)return null;
  return{
    name:'Shakata Yoga',category:'dosha',strength:46,strengthLabel:'Mild',
    formation:`Jupiter sits in House ${dist} counted from the Moon (House ${moonHouse}), one of the two positions (6th or 8th from Moon) that classically forms Shakata ("cart wheel") Yoga.`,
    signifies:'A pattern of fluctuating fortune, periods of gain followed by periods of struggle, like a cart wheel going up and down, particularly around finances and stability. Classical texts note this weakens considerably if Jupiter is otherwise well-dignified or angular from the Ascendant.',
    planetsInvolved:['Jupiter','Moon']
  };
}

// ---------------- ANGARAK DOSHA (Mars conjunct a lunar node) ----------------
function detectAngarakDosha(planetData){
  const withRahu=planetsConjunct('Mars','Rahu',planetData);
  const withKetu=planetsConjunct('Mars','Ketu',planetData);
  if(!withRahu&&!withKetu)return null;
  const node=withRahu?'Rahu':'Ketu';
  return{
    name:'Angarak Dosha',category:'dosha',strength:57,strengthLabel:'Mild to moderate',
    formation:`Mars and ${node} are conjunct in House ${houseOf('Mars',planetData)} (${SIGNS[planetData.Mars.sign]}).`,
    signifies:'An amplified, sometimes reckless intensity, impulsive action, accident-proneness, or conflict-seeking energy around this house\'s themes, though it can also manifest as unusual courage and drive when the rest of the chart supports it.',
    planetsInvolved:['Mars',node]
  };
}
...
function detectAllDoshas(planetData,lagnaSign){
  ...
  const dy=detectDaridraYoga(planetData,lagnaSign);if(dy)all.push(dy);
  const gd=detectGrahanDosha(planetData);if(gd)all.push(gd);
  const vy=detectVishYoga(planetData);if(vy)all.push(vy);
  const sy=detectShakataYoga(planetData);if(sy)all.push(sy);
  const ang=detectAngarakDosha(planetData);if(ang)all.push(ang);
  return all.sort((a,b)=>b.strength-a.strength);
}
...
const KARANA_NATURE={Bava:'Auspicious',Balava:'Auspicious',Kaulava:'Auspicious',Taitila:'Auspicious',Garaja:'Mixed',Vanija:'Auspicious',Vishti:'Inauspicious',Shakuni:'Mixed',Chatushpada:'Mixed',Naga:'Mixed',Kimstughna:'Mixed'};
...
function generatePersonalitySnapshot(lagnaSign,ascNakName,moonNakName){
  ...
  let body=`<div class="ps-row"><span class="ps-label">To the outside world</span> (Lagna nakshatra: ${ascNakName}) you tend to come across as ${lagnaTrait}, carrying ${lagnaFlavor}.</div>`;
  if(sameNak){
    body+=`<div class="ps-row">Your Moon shares this same nakshatra, which means your outer presentation and inner emotional world are unusually aligned, what people see is close to what you actually feel.</div>`;
  }else{
    body+=`<div class="ps-row"><span class="ps-label">Internally</span> (Moon nakshatra: ${moonNakName}) you are emotionally driven by something different, ${moonTrait}. This is the part of you that rarely shows on the surface but quietly steers how you feel and what you need to feel secure.</div>`;
  }
  body+=`<div class="ps-row" style="color:#888;font-size:11.5px;margin-top:10px">Based on Lagna nakshatra, Moon nakshatra, and Lagna sign only, a starting read, not the full chart.</div>`;
  const html=`<div class="ps-header" onclick="togglePersonalitySnapshot()">
    <span class="ps-title">A quick snapshot, outer vs. inner</span>
    <span class="ps-chevron" id="ps-chevron">&#9662;</span>
  </div>
  <div class="ps-body hidden" id="ps-body">${body}</div>`;
  return html;
}
...
// Toggles the collapsible body of the Quick Snapshot card open/closed on click, flipping the
// chevron glyph to match.
function togglePersonalitySnapshot(){
  const body=document.getElementById('ps-body');
  const chevron=document.getElementById('ps-chevron');
  if(!body)return;
  const nowHidden=body.classList.toggle('hidden');
  if(chevron)chevron.innerHTML=nowHidden?'&#9662;':'&#9652;';
}
