...
// The sampling half-window (days) is tuned PER PLANET to its typical daily motion, so every planet's
// window spans a comparable ~0.4-0.5deg of arc rather than a fixed number of hours:
// - Too WIDE for a fast planet (Mercury ~1.5deg/day) risks the window spanning an entire station and
//   averaging across it, reading the wrong direction (the original +/-1 day bug).
// - Too NARROW for a slow planet (Jupiter ~0.08deg/day, Saturn slower still) samples an angular
//   distance small enough to be dominated by the VSOP series' own truncation/precision noise, which
//   corrupts the derivative's SIGN even though the position itself is still accurate - this is what
//   was producing wrong Jupiter (and occasionally Mercury, right at its own faster stations) flags
//   with a single fixed window. Scaling the window to each planet's speed keeps the sampled arc well
//   above that noise floor for slow planets while staying tight enough not to straddle stations for
//   fast ones.
const MOTION_SAMPLE_HALF_DAY={Mercury:0.15,Venus:0.25,Mars:0.5,Jupiter:2.5,Saturn:5};
function getMotionInfo(planet,jd,ayanamsa){
  if(planet==='Sun'||planet==='Moon')return{retrograde:false,kendraDist:null};
  const H=MOTION_SAMPLE_HALF_DAY[planet]||0.3;
