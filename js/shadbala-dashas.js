...
// through the Chaldean order (Saturn, Jupiter, Mars, Sun, Venus, Mercury, Moon), 24 Horas per day
// (12 day + 12 night), using the app's existing 6am/6pm sunrise/sunset approximation for consistency
// with the rest of Kala Bala.
const CHALDEAN_ORDER=['Saturn','Jupiter','Mars','Sun','Venus','Mercury','Moon'];
function horaLord(weekday,hourDecimal){
  const dayLord=varshaMasaDinaHoraBala(weekday);
  const startIdx=CHALDEAN_ORDER.indexOf(dayLord);
  // Hours elapsed since 6am sunrise approximation, each Hora treated as 1 clock hour for simplicity
  // (true Horas are sunrise-to-sunset/12 and sunset-to-sunrise/12, which vary by season and latitude;
  // this app does not compute exact local sunrise, so it reuses the same 6am/6pm approximation already
  // used for isDayBirth elsewhere in Kala Bala, applied as 24 equal 1-hour Horas from 6am to 6am).
  const hoursSinceSixAM=((hourDecimal-6)+24)%24;
  const horaIdx=(startIdx+Math.floor(hoursSinceSixAM))%7;
  return CHALDEAN_ORDER[horaIdx];
}
// Varsha Bala (year lord, 15V - the smallest Varshadi Bala sub-component). Classical calculation derives
// the year lord from the Ahargana (elapsed days since an ancient epoch) - a precise but involved
// calculation. As a disclosed simplification, this app derives the year lord from the weekday the
// civil year began on January 1st of the birth year, cycling through the same weekday-lord sequence
// (this approximates, but does not exactly reproduce, the classical Saavana-day-based Abdadhipati rule).
function varshaLord(birthYear){
  const weekdayLords=['Sun','Moon','Mars','Mercury','Jupiter','Venus','Saturn'];
  const jan1=new Date(birthYear,0,1);
  return weekdayLords[jan1.getDay()];
}
...
function kalaBala(planet,hourDecimal,moonLon,sunLon,weekday,declination,isDayBirth,birthYear){
  ...
  const horaBalaLord=horaLord(weekday,hourDecimal);
  const hora=horaBalaLord===planet?60:0; // Hora bala - BPHS 27.13: 60V to the planetary-hour lord (largest Varshadi sub-component)
  const varshaBalaLord=birthYear?varshaLord(birthYear):null;
  const varsha=varshaBalaLord===planet?15:0; // Varsha bala - BPHS 27.13: 15V to the year lord (smallest Varshadi sub-component)
...
function calcShadbala(planetData,houseMap,birthInfo){
  const{hourDecimal,weekday,isDayBirth,navamsaSigns,retrograde,declinations,speedRatios,birthYear}=birthInfo;
  const planetHouses={};
  SHADBALA_PLANETS.forEach(p=>{planetHouses[p]=planetData[p].house});
  const results={};
  SHADBALA_PLANETS.forEach(planet=>{
    ...
    const kala=kalaBala(planet,hourDecimal,planetData.Moon.lon,planetData.Sun.lon,weekday,declinations[planet]||0,isDayBirth,birthYear);
