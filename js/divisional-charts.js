...
// ---------------- D2: HORA (wealth) ----------------
// Continuous Hora count: each sign's two 15deg halves are two sequential steps in a count that
// cycles through all 12 signs (not the simplified "always Cancer or Leo" binary Hora some software
// uses) - for ODD signs the count runs in zodiacal order, for EVEN signs it reverses. Verified against
// a real Jagannatha Hora D2 chart: all 9 grahas matched this formula exactly, while the previous
// Cancer/Leo-only formula matched none of them (JHora's actual output spans 6 different signs).
function getHoraSign(siderealLon){
  ...
  return isOdd?(signIdx*2+half)%12:(signIdx*2+1-half)%12;
}
