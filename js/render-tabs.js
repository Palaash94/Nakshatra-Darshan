...
function renderNakshatraKoota(){
  ...
  grid.innerHTML=`
    <div class="koota-hero-card">
      ...
      <div class="koota-hero-content">
        <div class="koota-hero-head">
          <div class="koota-hero-title">Birth Star (Janma Nakshatra)</div>
          <div class="koota-hero-nak">${moonNakName} <span class="koota-hero-pada">Pada ${moonPada} <span class="info-icon" onclick="showPadaInfo(${planetData.Moon.nakshatra.idx},${moonPada},'${moonNakName}')">i</span></span></div>
          <div class="koota-hero-sub">Determined by the Moon's position at birth — the basis for naming, personality reading, and marriage compatibility (Ashtakoot) in classical practice.</div>
        </div>
    ...
    <div class="koota-card">
      ...
      <div class="koota-row"><span class="koota-key">Vashya</span><span class="koota-val">${moonRashiVarna.vashyaInfo.label}</span></div>
      <div class="koota-desc">${moonRashiVarna.vashyaInfo.desc}</div>
...
function renderPlanetTable(pd,lagnaSign,asc){
  ...
  let rows=`<tr><td>Ascendant</td><td>${SIGNS[lagnaSign]}</td><td>—</td><td>${getDeg(asc).toFixed(1)}°</td><td>${nakPadaCell(ascNak)}</td><td>—</td></tr>`;
  rows+=PLANETS.map(p=>{
    const d=pd[p];
    ...
    return`<tr><td>${p}</td><td>${SIGNS[d.sign]}</td><td>H${d.house}</td><td>${d.deg.toFixed(1)}°</td><td>${nakPadaCell(d.nakshatra)}</td><td>${relLabel}</td></tr>`;
  }).join('');
...
let vargaChartsCache=null;

function renderVargasTab(){
  if(!chartData)return;
  if(!vargaChartsCache||vargaChartsCache._forName!==chartData.name+chartData.dob){
    vargaChartsCache=calcAllDivisionalCharts(chartData.planetData,chartData.ascSid,chartData.lagnaSign,chartData.houseMap);
    vargaChartsCache._forName=chartData.name+chartData.dob;
  }
  ...
  chipRow.innerHTML=order.map(key=>{
    const def=vargaChartsCache[key];
...
function renderVargaDetail(key){
  if(!vargaChartsCache||!vargaChartsCache[key])return;
  const chart=vargaChartsCache[key];
  ...
  container.innerHTML=`
    ...
    <div class="varga-legend">
      <span class="varga-legend-item"><span class="varga-legend-dot" style="background:#4ade8090"></span>Benefic-leaning aspect</span>
      <span class="varga-legend-item"><span class="varga-legend-dot" style="background:#f8717190"></span>Malefic-leaning aspect</span>
    </div>
    ...
    <table class="planet-table">
      ...
      <tbody>${PLANETS.map(p=>{
        ...
        return`<tr><td><span class="table-planet-cell">${planetIconSvg(p,14,'#f59e0b',1)} ${p}</span></td><td>${SIGNS[d1Sign]}</td><td>${SIGNS[vSign]}</td><td>${same?'<span class="rel-badge rel-friend">Yes</span>':'<span class="rel-badge rel-neutral">No</span>'}</td></tr>`;
      }).join('')}</tbody>
    </table>
    </div>
  `;
...
function showVargaSegmentDetail(house){
  const chart=vargaChartsCache[activeVargaKey];
  if(!chart)return;
...
function renderArgalaTab(){
  ...
  for(let h=1;h<=12;h++){
    ...
    cards.push(`<div class="karaka-card">
      <div class="karaka-card-head">
        <div class="seg-rashi-icon" style="width:26px;height:26px">${signIconSvg(signIdx,26,'#f59e0b',1)}</div>
        <div class="karaka-card-titles">
...
function renderTransitTab(){
  ...
  container.innerHTML=`
    ...
    <div class="varga-legend">
      <span class="varga-legend-item"><span class="varga-legend-dot" style="background:#f59e0b"></span>Natal position</span>
...
// =====================================================================
// LIFE MAP TAB (8-domain strength radar)
// =====================================================================
function renderLifeMapRadarSvg(domains){
  const cx=150,cy=150,maxR=110,n=domains.length;
  function pt(i,r){
    const ang=(-90+i*(360/n))*Math.PI/180;
    return{x:cx+r*Math.cos(ang),y:cy+r*Math.sin(ang)};
  }
  let svg=`<svg viewBox="0 0 300 320" style="width:100%;height:auto;max-width:420px;display:block;margin:0 auto">`;
  [0.25,0.5,0.75,1].forEach(f=>{
    const pts=domains.map((d,i)=>pt(i,maxR*f));
    svg+=`<polygon points="${pts.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ')}" fill="none" stroke="#2a2a2a" stroke-width="0.7"/>`;
  });
  domains.forEach((d,i)=>{
    const p=pt(i,maxR);
    svg+=`<line x1="${cx}" y1="${cy}" x2="${p.x.toFixed(1)}" y2="${p.y.toFixed(1)}" stroke="#2a2a2a" stroke-width="0.7"/>`;
    const labelP=pt(i,maxR+18);
    svg+=`<text x="${labelP.x.toFixed(1)}" y="${labelP.y.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" font-size="10" fill="#999" font-family="-apple-system,sans-serif">${d.label}</text>`;
  });
  const dataPts=domains.map((d,i)=>pt(i,(d.score/100)*maxR));
  svg+=`<polygon points="${dataPts.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ')}" fill="#f59e0b33" stroke="#f59e0b" stroke-width="1.6"/>`;
  dataPts.forEach(p=>{svg+=`<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3" fill="#f59e0b"/>`});
  svg+=`</svg>`;
  return svg;
}

let lifeMapCache=null;
function renderLifeMapTab(){
  if(!chartData)return;
  const cacheKey=chartData.name+chartData.dob;
  if(!lifeMapCache||lifeMapCache._forName!==cacheKey){
    lifeMapCache={data:calcLifeMap(chartData),_forName:cacheKey};
  }
  const domains=lifeMapCache.data;
  const gradeClass={Excellent:'av-good',Good:'av-good',Moderate:'av-mixed','Needs Attention':'av-bad'};
  const cards=domains.map(d=>`<div class="shadbala-card">
    <div class="shadbala-card-head">
      <div style="flex:1">
        <div class="shadbala-planet-name">${d.label}</div>
        <div class="seg-sub">${d.explanation}</div>
      </div>
      <div class="shadbala-status ${gradeClass[d.grade]}">${d.grade} · ${d.score}</div>
    </div>
  </div>`).join('');
  const container=document.getElementById('lifemap-content');
  container.innerHTML=`
    <div class="shadbala-intro">
      <span class="info-icon" id="lifemap-info-trigger">i</span>
      <span style="font-size:12.5px;color:#888">Blends Bhava Bala + Shadbala + detected Yogas/Doshas into one score per life theme</span>
    </div>
    ${renderLifeMapRadarSvg(domains)}
    <div class="section-title" style="margin-top:1.2rem">Domain breakdown</div>
    <div class="shadbala-grid">${cards}</div>
  `;
  document.getElementById('lifemap-info-trigger').onclick=function(){showGenericInfo('Life Map Methodology',LIFE_MAP_METHODOLOGY)};
}
...
function renderCompatibilityTab(){
  ...
  const verdictColor={Excellent:'#4ade80',Good:'#4ade80',Marginal:'#f59e0b','Not recommended':'#f87171'}[result.verdict];

  const rows=result.kootas.map(k=>{
    const pct=(k.score/k.max)*100;
    const barColor=pct>=70?'#4ade80':pct>=35?'#f59e0b':'#f87171';
    return`<div class="shadbala-card">
      <div class="shadbala-card-head">
        <div style="flex:1">
      ...
      <div class="dasha-bar-wrap" style="height:6px;margin-top:8px"><div class="dasha-bar" style="width:${pct}%;background:${barColor}"></div></div>
    </div>`;
  }).join('');
...
function showTab(name){
  const tabs=['chart','positions','dashas','karakas','yogadosha','shadbala','avastha','navamsa','vargas','transit','argala','lifemap','compatibility','reading'];
